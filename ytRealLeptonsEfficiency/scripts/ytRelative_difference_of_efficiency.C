...
void ytRelative_difference_of_efficiency(float pt_low = 10., float pt_up = 200., bool eta_less_than_two = false)
{
	...
	TString path = "~/Desktop/skim/Results/1026/";

	TFile *data_Zee = TFile::Open(path + "submitDir_Data_electron/hist-0929_80mll100.root");
	TFile *data_Zmumu = TFile::Open(path + "submitDir_Data_muon/hist-0929_80mll100.root");
	TFile *mc_ttbar_elec = TFile::Open(path + "submitDir_MC_ttbar_electron/hist-0929_80mll100.root");
	TFile *mc_ttbar_muon = TFile::Open(path + "submitDir_MC_ttbar_muon/hist-0929_80mll100.root");
	TFile *mc_GG_ttn1_elec = TFile::Open(path + "submitDir_MC_GG_ttn1_electron/hist-0929_80mll100.root");
	TFile *mc_GG_ttn1_muon = TFile::Open(path + "submitDir_MC_GG_ttn1_muon/hist-0929_80mll100.root");
	...
	TH3F *h_baseline_pt_eta_dRjet_Zee = 0, *h_baseline_pt_eta_dRjet_ttbar_elec = 0, *h_baseline_pt_eta_dRjet_GG_ttn1_elec = 0;
	TH3F *h_signal_pt_eta_dRjet_Zee = 0, *h_signal_pt_eta_dRjet_ttbar_elec = 0, *h_signal_pt_eta_dRjet_GG_ttn1_elec = 0;
	...
	if (!eta_less_than_two) { // electron uses all |eta| range
		...
		h_baseline_pt_eta_dRjet_Zee = (TH3F *)data_Zee->Get("h_baseline_pt_eta_dRjet");
		h_baseline_pt_eta_dRjet_ttbar_elec = (TH3F *)mc_ttbar_elec->Get("h_baseline_pt_eta_dRjet");
		h_baseline_pt_eta_dRjet_GG_ttn1_elec = (TH3F *)mc_GG_ttn1_elec->Get("h_baseline_pt_eta_dRjet");
		...
		int eta_bin_low = h_signal_pt_eta_dRjet_Zee->GetYaxis()->FindBin(0 + 0.01);
		int eta_bin_up = h_signal_pt_eta_dRjet_Zee->GetYaxis()->FindBin(2 - 0.01);
	...
	TH1F *h_eff_dRjet_Zee = (TH1F *)h_signal_dRjet_Zee->Clone();
	TH1F *h_eff_dRjet_Zmumu = (TH1F *)h_signal_dRjet_Zmumu->Clone();
	TH1F *h_eff_dRjet_ttbar_elec = (TH1F *)h_signal_dRjet_ttbar_elec->Clone();
	TH1F *h_eff_dRjet_ttbar_muon = (TH1F *)h_signal_dRjet_ttbar_muon->Clone();
	TH1F *h_eff_dRjet_GG_ttn1_elec = (TH1F *)h_signal_dRjet_GG_ttn1_elec->Clone();
	TH1F *h_eff_dRjet_GG_ttn1_muon = (TH1F *)h_signal_dRjet_GG_ttn1_muon->Clone();
...
*/
	...
	line2->Draw("same");
	TPaveText *text2 = new TPaveText(0.2, 0.8, 0.5, 0.85, "NDC");
	text2->AddText(pt_range.c_str());
	text2->SetTextSize(0.03);
	text2->Draw("same");
	TLegend *leg2 = new TLegend(0.2, 0.7, 0.5, 0.8);
	...
	h_relative_difference_eff_ttbar_elec->SetMaximum(1.0);
	h_relative_difference_eff_ttbar_elec->SetMinimum(-0.1);
	...
	TPaveText *text3 = new TPaveText(0.2, 0.8, 0.5, 0.85, "NDC");
	text3->AddText(pt_range.c_str());
	text3->SetTextSize(0.03);
	text3->Draw("same");
	TLegend *leg3 = new TLegend(0.2, 0.6, 0.5, 0.8);
	...
	TPaveText *text4 = new TPaveText(0.2, 0.4, 0.5, 0.45, "NDC");
	...
	leg4->AddEntry(h_average_eff_Zmumu, "Z#rightarrow #mu#mu (average)", "l");
	...
	line5->Draw("same");
	TPaveText *text5 = new TPaveText(0.2, 0.8, 0.5, 0.85, "NDC");
	text5->AddText(pt_range.c_str());
	text5->SetTextSize(0.03);
	text5->Draw("same");
	TLegend *leg5 = new TLegend(0.2, 0.7, 0.5, 0.8);
	...
	h_relative_difference_eff_ttbar_muon->SetMaximum(1.0);
	h_relative_difference_eff_ttbar_muon->SetMinimum(-0.1);
	...
	TPaveText *text6 = new TPaveText(0.2, 0.8, 0.5, 0.85, "NDC");
	text6->AddText(pt_range.c_str());
	text6->SetTextSize(0.03);
	text6->Draw("same");
	TLegend *leg6 = new TLegend(0.2, 0.6, 0.5, 0.8);
